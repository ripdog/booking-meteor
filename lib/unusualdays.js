checkBounds = function(thisobj) {
	...
	if(thisobj.key ==="startTime") {
		var startDate = cleanDate.clone().zone(-12).hour(thisobj.value).toDate();
		var endDate = cleanDate.clone().zone(-12).hour(thisobj.field("endTime").value).toDate();
	} else {
		var startDate = cleanDate.clone().zone(-12).hour(thisobj.field("startTime").value).toDate();
		var endDate = cleanDate.clone().zone(-12).hour(thisobj.value).toDate();
	}
