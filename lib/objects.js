...
checkDate = function(thisobj, isAppnt) {
	...
	/////////////CHECKING DAY BOUNDS
	/////////////////////////////////////////////////
	console.log(thisobj.field("providerName").value);
	...
	if (typeof provObject === "undefined" && Meteor.isClient) {
		provObject = getProvObject(cleanDate.toDate(), Session.get("selectedProviderName"));
	}
	console.log("performing checkDate, cleanDate: ", cleanDate.format(), ", thisobj: ", thisobj);
	//thisobj may fail due to timezone
	console.log("checking "+moment(thisobj.value).format()+
	" is inside "+ moment(cleanDate).tz("Pacific/Auckland").hours(provObject.startTime).format()
	+ " and "+ moment(cleanDate).tz("Pacific/Auckland").hours(provObject.endTime).format());
