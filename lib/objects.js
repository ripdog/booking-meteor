getBlockouts = function(provId, date) {
	...
	try {var providerBlockouts = providers.findOne({_id: provId/*, "blockouts.day": day*/}, {fields: {blockouts: 1}}).blockouts;}
	catch (e) {}
	var startDate = moment(date).startOf('day');
	var endDate = moment(date).endOf('day');
	var singleDayBlockouts = blockouts.find({date: {$gte: startDate, $lt: endDate}, providerID:provId}).fetch();
	var ret = _.union(providerBlockouts,/* anyDayBlockouts,*/ singleDayBlockouts);
...
checkDate = function(thisobj) {
	...
	var provObject = unusualDays.findOne({date: cleanDate.toDate(), providerID: thisobj.field("providerID").value});
	if (typeof provObject === "undefined") {
		provObject = providers.findOne(thisobj.field("providerID").value);
	}
	...
	console.log(JSON.stringify({date: {$gte: queryStart, $lt:queryEnd},providerID: thisobj.field("providerID").value}));
	var appoints = appointmentList.find({date: {$gte: queryStart, $lt:queryEnd},providerID: thisobj.field("providerID").value}).fetch();
	var ret;
	...
	/////////////////////////////////////////////////
	var blockouts = getBlockouts(thisobj.field("providerID").value,
		cleanDate.toDate());
