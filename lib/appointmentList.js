...
//this may fail due to timezone? try on nz timezone server.
			...
			// 	//if()
			// 	var currentAppoint = appoints[appointIndex];
			// 	for(var compareAppointIndex in appoints) {
			// 		var comparedAppoint = appoints[compareAppointIndex];
			// 		console.log("Comparing " + currentAppoint.date + " with " + comparedAppoint.date);
			// 		if (!(comparedAppoint._id === currentAppoint._id)) {
			// 				var range = moment(currentAppoint.date).twix(
			// 					moment(currentAppoint.date).add(currentAppoint.length, 'minutes'));
			// 				console.log(range);
			// 		}
			// 	}
			// 	// console.log(appoints[appoint]);
			// } 
		}
