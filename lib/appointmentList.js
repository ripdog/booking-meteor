...
appointmentList.attachSchema(new SimpleSchema({
	...
	date: {
		...
		custom: function() {
			...
			console.log("checking "+moment(this.value).format()+" is inside "+ cleanDate.tz("Pacific/Auckland").hours(provObject.startTime).format() + " and "+
				cleanDate.tz("Pacific/Auckland").hours(provObject.endTime).format());
			if(moment(this.value).isValid() === false) {
				return "wtf"
			}

			else if (moment(this.value).isBefore(cleanDate.tz("Pacific/Auckland").hours(provObject.startTime).utc())) {
				return "dateOutOfBounds"
			}
			else if (moment(this.value).isAfter(cleanDate.tz("Pacific/Auckland").hours(provObject.endTime).utc())) {
				return "dateOutOfBounds"
			}
